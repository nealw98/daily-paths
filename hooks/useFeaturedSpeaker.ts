...
// weeks since this anchor, so every user sees the same featured speaker in
// any given calendar week (timezone differences may shift the moment of
// transition by a few hours but not the chosen speaker within a week).
 ...
 *
 * Deterministic, global rotation: one speaker per week, identical for every
...
export function useFeaturedSpeaker(speakers: Speaker[]): Speaker | null {
  ...
  return useMemo(() => {
    ...
    const weekIndex = Math.floor((Date.now() - ROTATION_EPOCH_MS) / WEEK_MS);
